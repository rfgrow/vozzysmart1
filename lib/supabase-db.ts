...
export const campaignDb = {
  getAll: async (): Promise<Campaign[]> => {
    ...
    return (data || []).map((row) => ({
      ...
      templateVariables: row.template_variables as
        | {
            header: string[];
      ...
      qstashScheduleEnqueuedAt:
        (row as any).qstash_schedule_enqueued_at ?? null,
  ...
  getById: async (id: string): Promise<Campaign | undefined> => {
    ...
    return {
      ...
      templateVariables: data.template_variables as
        | {
            header: string[];
            headerMediaId?: string;
            body: string[];
      ...
      qstashScheduleEnqueuedAt:
        (data as any).qstash_schedule_enqueued_at ?? null,
  ...
  duplicate: async (id: string): Promise<Campaign | undefined> => {
    ...
    const recipientsCount =
      existingContacts?.length ?? original.recipients ?? 0;
  ...
  updateStatus: async (
    ...
    if (updates.firstDispatchAt !== undefined)
      updateData.first_dispatch_at = updates.firstDispatchAt;
    if (updates.lastSentAt !== undefined)
      updateData.last_sent_at = updates.lastSentAt;
    if (updates.scheduledAt !== undefined)
      updateData.scheduled_date = updates.scheduledAt;
    if (updates.qstashScheduleMessageId !== undefined)
      updateData.qstash_schedule_message_id = updates.qstashScheduleMessageId;
    if (updates.qstashScheduleEnqueuedAt !== undefined)
      updateData.qstash_schedule_enqueued_at = updates.qstashScheduleEnqueuedAt;
    if (updates.templateSnapshot !== undefined)
      updateData.template_snapshot = updates.templateSnapshot;
    if (updates.templateSpecHash !== undefined)
      updateData.template_spec_hash = updates.templateSpecHash;
    if (updates.templateParameterFormat !== undefined)
      updateData.template_parameter_format = updates.templateParameterFormat;
    if (updates.templateFetchedAt !== undefined)
      updateData.template_fetched_at = updates.templateFetchedAt;
...
export const contactDb = {
  ...
  list: async (params: {
    ...
    let suppressionMap = new Map<
      string,
      { reason: string | null; source: string | null; expiresAt: string | null }
    >();
    ...
    return {
      data: (data || []).map((row) => {
        const suppression =
          suppressionMap.get(String(row.phone || "").trim()) || null;
        return {
          id: row.id,
          name: row.name,
          phone: row.phone,
  ...
  /**
   * Busca quais telefones de uma lista já existem no banco.
   * Usa admin client (service_role) para evitar limites de RLS.
   */
  findPhones: async (phones: string[]): Promise<string[]> => {
    ...
    const { data, error } = await supabase
      ...
      .in("phone", phones);
    if (error) throw error;
    return (data || []).map((row: any) => String(row.phone));
  },
  ...
  add: async (
    ...
    if (existing) {
      ...
      if (contact.custom_fields)
        updateData.custom_fields = contact.custom_fields;
  ...
  import: async (
    contacts: Omit<Contact, "id" | "lastActive">[],
    ...
    try {
      ...
      console.error('[contactDb.import] Error creating timestamp:', error);
      throw new Error('Failed to create timestamp');
    }

    console.log(
      `[contactDb.import] Starting import of ${contacts.length} contacts`,
    );

    // Helper: Buscar contatos existentes em chunks para evitar query muito grande
    const fetchExistingContactsInChunks = async (
      phones: string[],
    ): Promise<Map<string, any>> => {
      const existingMap = new Map();

      for (let i = 0; i < phones.length; i += PHONE_CHUNK_SIZE) {
        const phoneChunk = phones.slice(i, i + PHONE_CHUNK_SIZE);
        ...
        if (error) {
          console.error(
            `[contactDb.import] Error fetching existing contacts chunk ${i}-${i + phoneChunk.length}:`,
            error,
          );
          throw error;
        }
    ...
    // Buscar todos os contatos existentes em chunks
    const phones = contacts.map((c) => c.phone).filter(Boolean);
    const existingByPhone = await fetchExistingContactsInChunks(phones);

    console.log(
      `[contactDb.import] Found ${existingByPhone.size} existing contacts`,
    );
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      console.log(
        `[contactDb.import] Processing batch ${batchIndex + 1}/${totalBatches} (${batch.length} contacts)`,
      );
      ...
      batch.forEach((contact) => {
        ...
        if (existing) {
          ...
          const newCustomFields =
            ...
            typeof (contact as any).custom_fields === "object"
              ? (contact as any).custom_fields
              : {};
      ...
      // Inserir novos contatos deste lote
      if (toInsert.length > 0) {
        ...
        if (insertError) {
          console.error(
            `[contactDb.import] Batch ${batchIndex + 1} insert error:`,
            insertError,
          );
          throw insertError;
        }
        ...
        console.log(
          `[contactDb.import] Batch ${batchIndex + 1}: Inserted ${toInsert.length} contacts`,
        );
      }

      // Atualizar contatos existentes deste lote
      if (toUpdate.length > 0) {
        ...
        if (updateError) {
          console.error(
            `[contactDb.import] Batch ${batchIndex + 1} update error:`,
            updateError,
          );
          throw updateError;
        }
        ...
        console.log(
          `[contactDb.import] Batch ${batchIndex + 1}: Updated ${toUpdate.length} contacts`,
        );
      }
    }

    console.log(
      `[contactDb.import] Completed: ${totalInserted} inserted, ${totalUpdated} updated`,
    );
  ...
  getStats: async () => {
    ...
    console.log('[contactDb.getStats] Raw data from RPC:', data);
    ...
    console.log('[contactDb.getStats] Returning:', result);
...
export const leadFormDb = {
  ...
  update: async (
    ...
    if ((dto as any).collectEmail !== undefined)
      updateData.collect_email = (dto as any).collectEmail;
    if (dto.successMessage !== undefined)
      updateData.success_message = dto.successMessage;
    if ((dto as any).fields !== undefined)
      updateData.fields = (dto as any).fields;
    updateData.updated_at = new Date().toISOString();
...
const normalizeTemplateFormat = (
  format: unknown,
): TemplateComponent["format"] | undefined => {
...
const normalizeTemplateComponents = (input: unknown): TemplateComponent[] => {
  ...
  if (header) {
    if (typeof header === "string") {
      ...
      if (header.example !== undefined)
        headerComponent.example = header.example;
  ...
  if (footer) {
    if (typeof footer === "string") {
      ...
      const footerText =
        typeof footer.text === "string" ? footer.text : undefined;
      if (footerText) {
...
const getTemplateBodyText = (
  ...
  if (raw && typeof raw === "object") {
    ...
    if (maybe.body && typeof maybe.body.text === "string")
      return maybe.body.text;
  }

  return "";
};

const normalizeParameterFormat = (
  value: unknown,
): "positional" | "named" | undefined => {
...
export const templateDb = {
  getAll: async (): Promise<Template[]> => {
    ...
    return (data || []).map((row) => {
      ...
      return {
        ...
        parameterFormat: normalizeParameterFormat(
          (row as any).parameter_format,
        ),
        ...
        headerMediaPreviewExpiresAt:
          (row as any).header_media_preview_expires_at ?? null,
        content: bodyText,
        preview: bodyText,
  ...
  getByName: async (name: string): Promise<Template | undefined> => {
    ...
    return {
      ...
      headerMediaPreviewExpiresAt:
        (data as any).header_media_preview_expires_at ?? null,
      content: bodyText,
      preview: bodyText,
    ...
    const { error } = await supabase.from("templates").upsert(
      {
        ...
        parameter_format:
          normalizeParameterFormat((template as any).parameterFormat) ||
          "positional",
        components:
...
export const settingsDb = {
  ...
  saveAll: async (settings: AppSettings): Promise<void> => {
    ...
    await settingsDb.set(
      "isConnected",
      settings.isConnected ? "true" : "false",
    );
  },
};
...
export const templateProjectDb = {
  getAll: async (): Promise<TemplateProject[]> => {
    ...
    return (data || []).map((project) => {
      ...
      const approvedCount = items.filter(
        (i: { meta_status?: string }) => i.meta_status === "APPROVED",
      ).length;
  ...
  getById: async (
    id: string,
  ): Promise<TemplateProject & { items: TemplateProjectItem[] }> => {
    // Fetch project
    ...
    return {
      ...
      items: (items as TemplateProjectItem[]) || [],
    };
  },
  ...
  updateItem: async (
    id: string,
    updates: Partial<TemplateProjectItem>,
  ): Promise<TemplateProjectItem> => {
    const { data, error } = await supabase
      .from("template_project_items")
      .update(updates)
      .eq("id", id)
...
export const campaignFolderDb = {
  ...
  update: async (
    id: string,
    dto: UpdateCampaignFolderDTO,
  ): Promise<CampaignFolder | undefined> => {
    const updateData: Record<string, unknown> = {};
...
export const campaignTagDb = {
  ...
  addToCampaign: async (campaignId: string, tagId: string): Promise<void> => {
    const { error } = await supabase.from("campaign_tag_assignments").upsert(
      {
        campaign_id: campaignId,
        tag_id: tagId,
      },
