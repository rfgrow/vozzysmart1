...
import { NodeConfigPanel } from "@/components/builder/workflow/node-config-panel";
...
const WorkflowEditor = ({ params }: WorkflowPageProps) => {
  ...
  const generatingParam = searchParams?.get("generating") ?? null;
  ...
  const hasLoadedWorkflowRef = useRef<string | null>(null);
  const loadingWorkflowIdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (
      loadingWorkflowIdRef.current === workflowId ||
      hasLoadedWorkflowRef.current === workflowId
    ) {
      return;
    }

    const loadWorkflowData = async () => {
      const isGeneratingParam = generatingParam === "true";
      ...
      try {
        ...
        if (loadingWorkflowIdRef.current === workflowId) {
          loadingWorkflowIdRef.current = null;
        }
        hasLoadedWorkflowRef.current = workflowId;
      }
    };
